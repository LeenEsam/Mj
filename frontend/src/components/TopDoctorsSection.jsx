...
const FILTER_GROUPS = [
  ...
  { value: "obstetrics_gynecology", ar: "نساء وتوليد",       en: "OB/GYN"        },
  { value: "pediatrics",            ar: "طب الأطفال",        en: "Pediatrics"    },
  { value: "psychiatry",            ar: "الصحة النفسية",     en: "Mental Health" },
  { value: "nutrition",             ar: "التغذية",           en: "Nutrition"     },
  { value: "neonatology",           ar: "حديثي الولادة",     en: "Neonatology"   },
  { value: "lactation_consultant",  ar: "الرضاعة الطبيعية",  en: "Lactation"     },
];
...
const TopDoctorsSection = () => {
  ...
  const fetchTopDoctors = async () => {
    ...
    }).sort((a, b) => b.avgRating - a.avgRating).slice(0, 12);
  ...
  const getSpecLabel = (value) => {
    const found = SPECIALIZATIONS.find(s =>
      s.value === value ||
      s.ar.toLowerCase() === (value || "").toLowerCase() ||
      s.en.toLowerCase() === (value || "").toLowerCase()
    );
    if (found) return isAr ? found.ar : found.en;
    return value || (isAr ? "طبيب" : "Doctor");
  };
