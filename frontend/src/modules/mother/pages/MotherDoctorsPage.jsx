...
const SPECIALTY_KEYWORDS = {
  gynecology:   ["نسائية", "توليد", "gynecol", "obstet"],
  pediatrics:   ["أطفال", "رضع", "pediatric", "neonat"],
  psychiatry:   ["نفس", "اكتئاب", "قلق", "psychiatr", "mental", "psycholog"],
  nutrition:    ["تغذية", "غذائ", "nutritio", "dietit"],
};

const FILTER_LABELS = {
  ar: { all: "الكل", gynecology: "نسائية وتوليد", pediatrics: "طب الأطفال", psychiatry: "صحة نفسية", nutrition: "تغذية" },
  en: { all: "All",  gynecology: "Gynecology",    pediatrics: "Pediatrics",  psychiatry: "Mental Health", nutrition: "Nutrition" },
};
...
const MotherDoctorsPage = () => {
  ...
  const filtered = doctors.filter(d => {
    ...
    const keywords = SPECIALTY_KEYWORDS[filter] || [];
    return matchSearch && keywords.some(kw => spec.includes(kw));
  });

  const labels = isAr ? FILTER_LABELS.ar : FILTER_LABELS.en;
      ...
      <main className="mdr-main">
        ...
        <div className="mdr-filters">
          {Object.entries(labels).map(([key, lbl]) => (
            <button
              key={key}
              className={`mdr-filter-btn ${filter === key ? "active" : ""}`}
              onClick={() => setFilter(key)}
            >
              {lbl}
              {key === "all" && <span className="mdr-filter-count">{doctors.length}</span>}
            </button>
          ))}
        </div>
