...
const SPECIALIZATIONS = {
  ar: ["الكل", "نسائية وتوليد", "طب الأطفال", "صحة نفسية", "تغذية", "طب عام"],
  en: ["All", "Gynecology", "Pediatrics", "Mental Health", "Nutrition", "General Medicine"],
};
...
const DoctorsPage = () => {
  ...
  const [activeFilter, setActiveFilter] = useState(0); // index
 ...
 const fetchDoctors = async () => {
  ...
  const filters = isAr ? SPECIALIZATIONS.ar : SPECIALIZATIONS.en;
  const arFilters = SPECIALIZATIONS.ar;
  ...
  const filtered = doctors.filter((d) => {
    ...
    if (activeFilter === 0) return matchSearch;

    // Match by arabic specialization keyword regardless of display lang
    const arKey = arFilters[activeFilter].toLowerCase();
    const enKey = (SPECIALIZATIONS.en[activeFilter] || "").toLowerCase();
    const matchFilter = spec.includes(arKey) || spec.includes(enKey);
      ...
      <main className="dm-main">
        ...
        <div className="dm-filters">
          {filters.map((f, i) => (
            <button
              key={i}
              className={`dm-filter-btn ${activeFilter === i ? "active" : ""}`}
              onClick={() => setActiveFilter(i)}
            >
              {f}
              {i === 0 && <span className="dm-filter-count">{doctors.length}</span>}
            </button>
          ))}
        </div>
